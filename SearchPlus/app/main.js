...
function getCurrentMode() {
    return getModeById(getBackgroundModeId());
}

function changeModeTo(modeId) {
    let newMode = getModeById(modeId);
    if (!newMode) return;
    if (getBackgroundModeId() !== undefined) {
        getCurrentMode().onQuit();
    }
    newMode.onInit();
    setModeText(modeId);
    setBackgroudModeId(modeId);
}
...
$(document).ready(() => {
    $('#search-text').keypress(event => {
        switch (event.which) {
        case 10:
            getCurrentMode().onCtrlEnter();
            break;
        case 13:
